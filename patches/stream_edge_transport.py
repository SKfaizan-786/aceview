...
class StreamEdge(EdgeTransport[StreamCall]):
    ...
    async def create_call(self, call_id: str, **kwargs) -> StreamCall:
        ...
        # Do NOT call get_or_create here.
        # The frontend already created the call, and the coordinator /join endpoint
        # (called inside edge.join → rtc.join) handles creation atomically with create=True.
        # Calling get_or_create first is redundant and can hit a different SFU edge node
        # than the subsequent /join call, which causes "participant not found" on SetPublisher.
