...
import { ArrowLeft, Play, Square, Award, TrendingUp, Lightbulb, Download } from 'lucide-react';
...
export default function InterviewPage() {
  ...
  const handleDownloadPDF = () => {
    ...
    const grade = sessionSummary.grade;
    const gradeColor = grade === 'A' ? '#00b894' : grade === 'B' ? '#6c5ce7' : grade === 'C' ? '#fdcb6e' : '#e17055';

    win.document.write(`<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8"/>
  <title>AceView Report Card</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', system-ui, sans-serif; }
    body { background: #f0f4f8; color: #2d3436; padding: 40px; }
    .card { background: #fff; border-radius: 16px; padding: 32px; margin-bottom: 20px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); }
    .header { text-align: center; }
    .score-circle { width: 100px; height: 100px; border-radius: 50%; background: #e8edf2; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; font-size: 36px; font-weight: 800; color: #2d3436; }
    .grade { font-size: 26px; font-weight: 700; color: ${gradeColor}; margin-bottom: 8px; }
    .summary { color: #636e72; font-size: 15px; line-height: 1.6; max-width: 480px; margin: 0 auto; }
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
    h3 { font-size: 16px; font-weight: 700; margin-bottom: 14px; display: flex; align-items: center; gap: 8px; }
    ul { list-style: none; }
    li { font-size: 14px; color: #636e72; padding: 5px 0; display: flex; align-items: flex-start; gap: 8px; }
    .dot-green { color: #00b894; font-weight: 700; }
    .dot-yellow { color: #fdcb6e; font-weight: 700; }
    .tip { font-size: 14px; color: #636e72; line-height: 1.6; }
    .footer { text-align: center; color: #b2bec3; font-size: 12px; margin-top: 24px; }
    .brand { font-weight: 800; color: #6c5ce7; }
    .metrics { display: flex; justify-content: center; gap: 32px; margin-top: 16px; }
    .metric { text-align: center; }
    .metric-val { font-size: 22px; font-weight: 700; color: #2d3436; }
    .metric-lbl { font-size: 11px; color: #b2bec3; text-transform: uppercase; letter-spacing: 0.5px; }
  </style>
</head>
<body>
  <div class="card header">
    <div class="score-circle">${sessionSummary.overall_score}</div>
    <p class="grade">Grade: ${grade}</p>
    <p class="summary">${sessionSummary.summary}</p>
  </div>

  <div class="grid">
    <div class="card">
      <h3><span class="dot-green">✦</span> Strengths</h3>
      <ul>${sessionSummary.strengths.map(s => `<li><span class="dot-green">✓</span> ${s}</li>`).join('')}</ul>
    </div>
    <div class="card">
      <h3><span class="dot-yellow">✦</span> Areas to Improve</h3>
      <ul>${sessionSummary.improvements.map(s => `<li><span class="dot-yellow">→</span> ${s}</li>`).join('')}</ul>
    </div>
  </div>

  <div class="card">
    <h3>💡 Tip of the Day</h3>
    <p class="tip">${sessionSummary.tip_of_the_day}</p>
  </div>

  <p class="footer">Generated by <span class="brand">AceView AI Interview Coach</span> · ${new Date().toLocaleDateString()}</p>
      ...
      <div className="min-h-screen bg-[#e0e5ec]">
          ...
          <div className="max-w-7xl mx-auto px-6 py-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                ...
                className={`r-neu flex items-center justify-center gap-2 px-6 py-3 font-bold transition-all cursor-pointer ${isSessionActive ? 'neu-pressed text-[#e17055]' : 'neu text-[#00b894]'
                  }`}
              >
                {isSessionActive ? (
                  <>
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          {!isSessionActive && !isFetchingSummary && sessionSummary && (
            <div id="report-card" className="mt-12 max-w-3xl mx-auto space-y-6">
              {/* Score banner */}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 neu r-neu py-4 font-bold text-[#00b894] text-lg transition-all cursor-pointer"
                >
                  Practice Again
                </button>
                <button
                  onClick={handleDownloadPDF}
                  className="no-print neu r-neu py-4 px-6 font-bold text-[#6c5ce7] flex items-center gap-2 transition-all cursor-pointer"
                >
